...
function buildRoundedFilterComplex(
  ...
  for (let i = 0; i < replacements.length; i++) {
    ...
    textChain += `drawtext=text='${safeText}':font=${style.fontFamily}:fontsize=${currentFontSize}:fontcolor=0x${fontColorFFmpeg}:borderw=2:bordercolor=0x${fontColorFFmpeg}:x=${textX}:y=${textY}${enableExpression}`;
    addedText = true;
...
function buildTextOverlay(
  ...
  filter += `:borderw=2:bordercolor=${options.fontColor}`;
  filter += `:x=${options.x}`;
  filter += `:y=${options.y}`;
