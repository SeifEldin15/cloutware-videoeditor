...
export const tiktokStyleAnimation = (
  ...
  try {
    ...
    // Shake / move tag for the whole segment (calculated once, not per-word)
    let moveTag = '';
    if (style?.animation === 'shake') {
      try {
        const endPosition = calculateNextPosition(currentPosition.x, currentPosition.y, end - start);
    ...
    // Show all words in the segment fully highlighted in ONE event.
    // The word grouping (wordsPerGroup) is already handled upstream by processWordModeSegments —
    // creating one sub-event per word here would cause each group to appear N times (the duplication bug).
    const highlightedWords = words.map(w =>
      `{${moveTag}\\c${textColor}\\bord${outlineWidth}\\3c${outlineColorASS}\\blur${outlineBlur}\\shad0}${w}`
    ).join(' ');

    if (glowEnabled) {
      const borderWidth = Math.max(0.1, 0.1 * effectiveShadowStrength);
      const blurAmount  = Math.max(1,   3   * effectiveShadowStrength);
      const glowWords = words.map(w =>
        `{${moveTag}\\c${lightGlowColorASS}\\bord${borderWidth}\\blur${blurAmount}\\3c${lightGlowColorASS}\\4c${lightGlowColorASS}\\4a&H${blurAlpha.toString(16)}&\\3a&H${shadowAlpha.toString(16)}&}${w}`
      ).join(' ');
      events.push(`Dialogue: 1,${formatTime(start)},${formatTime(end)},Default,,0,0,0,,${glowWords}`);
    }
    events.push(`Dialogue: 2,${formatTime(start)},${formatTime(end)},Default,,0,0,0,,${highlightedWords}`);
