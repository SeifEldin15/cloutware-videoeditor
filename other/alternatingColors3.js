...
export const alternatingColorsAnimation3 = (subtitle, start, end, style, lastPosition = null) => {
  const words = subtitle.text.split(' ');
  const totalDuration = end - start;
  const wordGroups = [];
  const outlineWidth = getScaledOutlineWidth(style) * 0.7;
  ...
  const colorSets = style?.alternateColors ? [
    ...
    [convertColorToASS(style.alternateColors[2]), convertColorToASS(style.alternateColors[3])]
  ].map(set => set.map(color => color.replace('&H', '').replace('&', ''))) : defaultColorSets;

  // Get shadow colors from style
  const shadowColorSets = style?.alternateColors ? [
    [convertColorToASS(style.alternateColors[0]), convertColorToASS(style.alternateColors[1])],
    [convertColorToASS(style.alternateColors[2]), convertColorToASS(style.alternateColors[3])]
  ...
  const glowColors = {
    ...
    ...(style?.alternateColors && {
      ...
      [convertColorToASS(style.alternateColors[2]).replace('&H', '').replace('&', '')]: convertColorToASS(style.alternateColors[2]).replace('&H', '').replace('&', ''),
      [convertColorToASS(style.alternateColors[3]).replace('&H', '').replace('&', '')]: convertColorToASS(style.alternateColors[3]).replace('&H', '').replace('&', '')
    })
  };
  ...
  const glowAlphas = {
    ...
    ...(style?.alternateColors && {
      ...
      [convertColorToASS(style.alternateColors[2]).replace('&H', '').replace('&', '')]: '&HA3&',
      [convertColorToASS(style.alternateColors[3]).replace('&H', '').replace('&', '')]: '&HA5&'
    })
  };
  ...
  const glowBlur = {
    ...
    ...(style?.alternateColors && {
      ...
      [convertColorToASS(style.alternateColors[2]).replace('&H', '').replace('&', '')]: 19,
      [convertColorToASS(style.alternateColors[3]).replace('&H', '').replace('&', '')]: 20
    })
  };
  ...
  wordGroups.forEach((group, groupIndex) => {
    ...
    group.forEach((pair, pairIndex) => {
      ...
      // Calculate movement if animation2 is Shake
      let moveTag = '';
      ...
      events.push(`Dialogue: 0,${formatTime(pairStart)},${formatTime(pairEnd)},Default,,0,0,0,,${coloredText}`);
      events.push(`Dialogue: 1,${formatTime(pairStart)},${formatTime(pairEnd)},Default,,0,0,0,,${glowText}`);
    });
  });
